...
ActiveAccountInfo startaccount(char * accountname, int clientfd)
{
	...
	for(i; i < MAX_ACCOUNTS; i++)
	{
		...
		if(Bank[i].taken)
		{
			...
			if(!strcmp(Bank[i].account->name, accountname))
			{
				//try to lock the desired account
				//if it is already locked print an error message to the client
				//that the account is already in session
				if(pthread_mutex_trylock(&mutex_account[i]) != 0)
				{
					pthread_mutex_unlock(&mutex_bank);
